...
$(document).on("click", "#BTN_INPUT", (event) => {
    var email = $('#USEREMAIL').val();
    var pin  = prompt("Enter the PIN");
    var record = $('#RECORD').val();
    var target = "http://" + ip + ":4000/api/qnet/input";
    $.post(target,
        {
            email: email,
            pin: pin,
...
$(document).on("click", "#BTN_QUERY", (event) => {
    var email = $('#USEREMAIL').val();
    var pin  = prompt("Enter the PIN");
    var target = "http://" + ip + ":4000/api/qnet/query";
    $.post(target,
        {
            email: email,
            pin: pin
...
$(document).on("click", "#BTN_DELETE", (event) => {
    var email = $('#USEREMAIL').val();
    var pin  = prompt("Enter the PIN");
    var target = "http://" + ip + ":4000/api/qnet/delete";
    $.post(target,
        {
            email: email,
            pin: pin
